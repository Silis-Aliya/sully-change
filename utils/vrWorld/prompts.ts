...
export function buildVRSystemAddendum(room: VRRoomDef, charName: string): string {
    return [
        `\n\n=== 你登入了《彼方》 ===`,
        `《彼方》是一款真实存在、你自己在玩的 VR 社交游戏（就像 VRChat）：你有自己的账号，戴上设备就能登入，在里面用你的虚拟化身（就是你那个亲手捏的 Q 版小人模样）四处走动、进出不同房间、撞见别的玩家（其他角色的化身）。这不是做梦、也不是比喻——就是你拿自己的空闲时间在玩的一款游戏。它发生在游戏世界里，和你跟用户的现实相处是两码事、互不冲突；现在是你独自上线、没人盯着的私人时间。`,
